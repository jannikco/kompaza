...
<div class="bg-gray-800 rounded-xl border border-gray-700 p-6 max-w-2xl">
    <h2 class="text-lg font-semibold text-white mb-6">Edit Plan: <?= h($plan['name']) ?></h2>
    <form method="POST" action="/plans/update" class="space-y-4">
        <?= csrfField() ?>
        <input type="hidden" name="id" value="<?= $plan['id'] ?>">
        <div class="grid grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-medium text-gray-300 mb-1">Name</label>
                <input type="text" name="name" value="<?= h($plan['name']) ?>" required class="w-full bg-gray-700 border border-gray-600 text-white rounded-lg px-4 py-2">
            </div>
            <div>
        ...
        <div class="grid grid-cols-3 gap-4">
            <div>
                ...
                <input type="number" name="max_customers" value="<?= h($plan['max_customers'] ?? '') ?>" class="w-full bg-gray-700 border border-gray-600 text-white rounded-lg px-4 py-2">
            </div>
            <div>
                <label class="block text-sm text-gray-400 mb-1">Max Leads</label>
                <input type="number" name="max_leads" value="<?= h($plan['max_leads'] ?? '') ?>" class="w-full bg-gray-700 border border-gray-600 text-white rounded-lg px-4 py-2">
            </div>
            <div>
                <label class="block text-sm text-gray-400 mb-1">Max Campaigns</label>
                <input type="number" name="max_campaigns" value="<?= h($plan['max_campaigns'] ?? '') ?>" class="w-full bg-gray-700 border border-gray-600 text-white rounded-lg px-4 py-2">
            </div>
            <div>
                <label class="block text-sm text-gray-400 mb-1">Max Products</label>
                <input type="number" name="max_products" value="<?= h($plan['max_products'] ?? '') ?>" class="w-full bg-gray-700 border border-gray-600 text-white rounded-lg px-4 py-2">
            </div>
            <div>
                <label class="block text-sm text-gray-400 mb-1">Max Lead Magnets</label>
                <input type="number" name="max_lead_magnets" value="<?= h($plan['max_lead_magnets'] ?? '') ?>" class="w-full bg-gray-700 border border-gray-600 text-white rounded-lg px-4 py-2">
            </div>
            <div>
                <label class="block text-sm text-gray-400 mb-1">Sort Order</label>
                <input type="number" name="sort_order" value="<?= $plan['sort_order'] ?>" class="w-full bg-gray-700 border border-gray-600 text-white rounded-lg px-4 py-2">
            </div>
        </div>
        <div class="flex items-center gap-2 pt-2">
            <input type="checkbox" name="is_active" id="is_active" <?= $plan['is_active'] ? 'checked' : '' ?> class="rounded bg-gray-700 border-gray-600 text-indigo-600">
            <label for="is_active" class="text-sm text-gray-300">Active</label>
        </div>
        <div class="pt-4">
            <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-lg font-medium">Update Plan</button>
        </div>
    </form>
</div>
