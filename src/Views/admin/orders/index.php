...
$statusColors = [
    ...
    'processing' => 'bg-blue-100 text-blue-700 border-blue-700',
    'shipped' => 'bg-indigo-100 text-indigo-700 border-indigo-700',
    'delivered' => 'bg-green-100 text-green-700 border-green-700',
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        <td class="px-6 py-4">
                            <a href="/admin/ordrer/<?= $order['id'] ?>" class="text-sm font-medium text-indigo-600 hover:text-indigo-500">
                                #<?= h($order['order_number']) ?>
                            </a>
                        </td>
                        <td class="px-6 py-4">
                            <div class="text-sm font-medium text-gray-900"><?= h($order['customer_name'] ?? 'Guest') ?></div>
                            <div class="text-xs text-gray-500"><?= h($order['customer_email'] ?? '') ?></div>
                        </td>
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium <?= $statusBadgeColors[$order['status']] ?? 'bg-gray-100 text-gray-700' ?>">
                                <?= ucfirst($order['status']) ?>
                            </span>
                        </td>
                        <td class="px-6 py-4 text-sm font-medium text-gray-900"><?= formatMoney($order['total_dkk']) ?></td>
                        <td class="px-6 py-4">
                            <?php if (!empty($order['payment_method'])): ?>
                                <span class="text-sm text-gray-600"><?= h(ucfirst($order['payment_method'])) ?></span>
                            <?php else: ?>
                                <span class="text-sm text-gray-500">-</span>
                            <?php endif; ?>
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-500"><?= formatDate($order['created_at'], 'd M Y, H:i') ?></td>
        ...
        <div class="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
            ...
            <div class="flex items-center space-x-1">
                <?php if ($currentPageNum > 1): ?>
                    <a href="/admin/ordrer?page=<?= $currentPageNum - 1 ?><?= !empty($statusFilter) ? '&status=' . urlencode($statusFilter) : '' ?>" class="px-3 py-1.5 text-sm text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition">Previous</a>
                <?php endif; ?>
                ...
                <?php if ($currentPageNum < $totalPages): ?>
                    <a href="/admin/ordrer?page=<?= $currentPageNum + 1 ?><?= !empty($statusFilter) ? '&status=' . urlencode($statusFilter) : '' ?>" class="px-3 py-1.5 text-sm text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition">Next</a>
                <?php endif; ?>
            </div>
        </div>
        <?php endif; ?>
