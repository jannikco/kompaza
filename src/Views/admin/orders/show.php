...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        <a href="/admin/ordrer" class="text-sm text-gray-400 hover:text-white transition">&larr; Back to Orders</a>
        <div class="flex items-center gap-3 mt-1">
            <h2 class="text-2xl font-bold text-white">Order #<?= h($order['order_number']) ?></h2>
            <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium <?= $statusColors[$order['status']] ?? 'bg-gray-700 text-gray-300' ?>">
                <?= ucfirst($order['status']) ?>
            </span>
        </div>
        <p class="text-sm text-gray-400 mt-1">Placed on <?= formatDate($order['created_at'], 'd M Y \a\t H:i') ?></p>
    </div>
...
<div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
    <div class="bg-gray-800 border border-gray-700 rounded-xl p-4">
        ...
        <p class="text-xl font-bold text-white mt-1"><?= formatMoney($order['total_dkk']) ?></p>
    </div>
    <div class="bg-gray-800 border border-gray-700 rounded-xl p-4">
        <p class="text-xs font-medium text-gray-500 uppercase tracking-wider">Payment Method</p>
        <p class="text-xl font-bold text-white mt-1"><?= h(ucfirst($order['payment_method'] ?? 'N/A')) ?></p>
    </div>
    <div class="bg-gray-800 border border-gray-700 rounded-xl p-4">
        <p class="text-xs font-medium text-gray-500 uppercase tracking-wider">Items</p>
        <p class="text-xl font-bold text-white mt-1"><?= count($items ?? []) ?></p>
    </div>
    <div class="bg-gray-800 border border-gray-700 rounded-xl p-4">
        <p class="text-xs font-medium text-gray-500 uppercase tracking-wider">Currency</p>
        <p class="text-xl font-bold text-white mt-1"><?= h($order['currency'] ?? 'DKK') ?></p>
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
    ...
    <div class="space-y-6">
            ...
            <div class="space-y-3">
                <div class="flex items-center">
                    <div class="flex-shrink-0 w-10 h-10 bg-indigo-600 rounded-full flex items-center justify-center">
                        <span class="text-sm font-bold text-white"><?= h(mb_strtoupper(mb_substr($order['customer_name'] ?? '?', 0, 1))) ?></span>
                    </div>
                    <div class="ml-3">
                        <p class="text-sm font-medium text-white"><?= h($order['customer_name'] ?? 'Guest') ?></p>
                        <?php if (!empty($order['customer_email'])): ?>
                            <a href="mailto:<?= h($order['customer_email']) ?>" class="text-xs text-indigo-400 hover:text-indigo-300"><?= h($order['customer_email']) ?></a>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-200 mt-0.5"><?= h($order['customer_phone']) ?></p>
                </div>
                <?php endif; ?>
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-200 mt-0.5"><?= h($order['customer_company']) ?></p>
                </div>
                <?php endif; ?>

                <?php if (!empty($order['customer_id'])): ?>
                <div class="pt-2">
                    <a href="/admin/kunder/<?= $order['customer_id'] ?>" class="text-sm text-indigo-400 hover:text-indigo-300 inline-flex items-center">
        ...
        <div class="bg-gray-800 border border-gray-700 rounded-xl p-6">
            <h3 class="text-lg font-semibold text-white mb-3">Billing Address</h3>
            <p class="text-sm text-gray-300 whitespace-pre-line"><?= h($order['billing_address']) ?></p>
        </div>
        <?php endif; ?>
        ...
        <div class="bg-gray-800 border border-gray-700 rounded-xl p-6">
            <h3 class="text-lg font-semibold text-white mb-3">Shipping Address</h3>
            <p class="text-sm text-gray-300 whitespace-pre-line"><?= h($order['shipping_address']) ?></p>
        </div>
        <?php endif; ?>
        ...
        <div class="bg-gray-800 border border-gray-700 rounded-xl p-6">
            <h3 class="text-lg font-semibold text-white mb-3">Payment Reference</h3>
    ...
    <div class="lg:col-span-2 space-y-6">
                ...
                <table class="w-full">
                    <thead>
                        <tr class="border-b border-gray-700 bg-gray-800/50">
                            ...
                            <th class="px-6 py-3 text-center text-xs font-medium text-gray-400 uppercase tracking-wider">Qty</th>
                    ...
                    <tbody class="divide-y divide-gray-700">
                        <?php if (!empty($items)): ?>
                            ...
                            <tr class="hover:bg-gray-750 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="text-sm font-medium text-white"><?= h($item['product_name']) ?></div>
                                ...
                                <td class="px-6 py-4 text-sm text-gray-300 text-center"><?= (int)$item['quantity'] ?></td>
                                <td class="px-6 py-4 text-sm text-gray-300 text-right"><?= formatMoney($item['unit_price_dkk']) ?></td>
                                <td class="px-6 py-4 text-sm font-medium text-white text-right"><?= formatMoney($item['total_price_dkk']) ?></td>
                            </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="4" class="px-6 py-8 text-center text-gray-400 text-sm">No items in this order.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            ...
            <div class="border-t border-gray-700 px-6 py-4">
                <div class="max-w-xs ml-auto space-y-2">
                    <div class="flex justify-between text-sm">
                        ...
                        <span class="text-gray-200"><?= formatMoney($order['subtotal_dkk'] ?? $order['total_dkk']) ?></span>
                    </div>
                    <?php if (!empty($order['tax_dkk']) && $order['tax_dkk'] > 0): ?>
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-400">Tax (VAT)</span>
                        <span class="text-gray-200"><?= formatMoney($order['tax_dkk']) ?></span>
                    </div>
                    <?php endif; ?>
                    <?php if (!empty($order['shipping_dkk']) && $order['shipping_dkk'] > 0): ?>
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-400">Shipping</span>
                        <span class="text-gray-200"><?= formatMoney($order['shipping_dkk']) ?></span>
                    </div>
                    <?php endif; ?>
                    <?php if (!empty($order['discount_dkk']) && $order['discount_dkk'] > 0): ?>
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-400">Discount</span>
                        <span class="text-green-400">-<?= formatMoney($order['discount_dkk']) ?></span>
                    </div>
                    <?php endif; ?>
                    <div class="flex justify-between pt-2 border-t border-gray-700">
                        <span class="text-base font-semibold text-white">Total</span>
                        <span class="text-base font-bold text-white"><?= formatMoney($order['total_dkk']) ?></span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gray-800 border border-gray-700 rounded-xl p-6">
            ...
            <p class="text-sm text-gray-300 whitespace-pre-line"><?= h($order['notes']) ?></p>
        </div>
        <?php endif; ?>
    </div>
</div>
...
<div id="status-section" class="bg-gray-800 border border-gray-700 rounded-xl p-6 mb-6">
    ...
    <form method="POST" action="/admin/ordrer/status">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label for="new_status" class="block text-sm font-medium text-gray-300 mb-1.5">New Status</label>
                <select id="new_status" name="status" x-data="{ status: '<?= h($order['status']) ?>' }" x-model="status"
                        class="w-full px-4 py-2.5 bg-gray-700 border border-gray-600 text-white rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm">
            ...
            <div>
                <label for="note" class="block text-sm font-medium text-gray-300 mb-1.5">Note (optional)</label>
        ...
        <div x-data="{ showShipping: <?= in_array($order['status'], ['shipped', 'delivered']) ? 'true' : 'false' ?> }" class="mt-4">
            <div x-show="showShipping || document.getElementById('new_status')?.value === 'shipped'" x-cloak>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-gray-700">
                    <div>
                        <label for="tracking_number" class="block text-sm font-medium text-gray-300 mb-1.5">Tracking Number</label>
                        <input type="text" id="tracking_number" name="tracking_number" value="<?= h($order['tracking_number'] ?? '') ?>"
                               class="w-full px-4 py-2.5 bg-gray-700 border border-gray-600 text-white placeholder-gray-400 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm"
                               placeholder="Enter tracking number">
                    </div>
                    <div>
                        <label for="tracking_url" class="block text-sm font-medium text-gray-300 mb-1.5">Tracking URL</label>
...
<div class="bg-gray-800 border border-gray-700 rounded-xl p-6">
    <h3 class="text-lg font-semibold text-white mb-6">Status History</h3>
            ...
            <div class="relative flex items-start pl-10">
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-3">
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium <?= $statusColors[$history['status']] ?? 'bg-gray-700 text-gray-300' ?>">
                            <?= ucfirst($history['status']) ?>
                        </span>
                        <span class="text-xs text-gray-500"><?= formatDate($history['created_at'], 'd M Y, H:i') ?></span>
                    </div>
                    <?php if (!empty($history['note'])): ?>
                    <p class="text-sm text-gray-400 mt-1"><?= h($history['note']) ?></p>
                    <?php endif; ?>
                </div>
            </div>
            <?php endforeach; ?>
