...
<div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
    <?php if (empty($products)): ?>
        <div class="p-12 text-center">
            ...
            <p class="text-gray-500 mb-4">No products yet. Add your first product to start selling.</p>
            <a href="/admin/produkter/opret" class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg transition">
                Add Product
            </a>
        </div>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors" x-data="{ confirmDelete: false }">
                        <td class="px-6 py-4">
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    <div class="text-sm font-medium text-gray-900"><?= h($product['name']) ?></div>
                        ...
                        <td class="px-6 py-4 text-sm text-gray-600 font-mono"><?= h($product['sku'] ?? '-') ?></td>
                        <td class="px-6 py-4">
                            <div class="text-sm font-medium text-gray-900"><?= formatMoney($product['price_dkk']) ?></div>
                        ...
                        <td class="px-6 py-4">
                            <?php if ($product['track_stock']): ?>
                                ...
                                <?php elseif ($product['stock_quantity'] <= 5): ?>
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-700"><?= (int)$product['stock_quantity'] ?> left</span>
                                <?php else: ?>
                                    <span class="text-sm text-gray-600"><?= (int)$product['stock_quantity'] ?></span>
                                <?php endif; ?>
                            <?php else: ?>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end space-x-2">
                                <a href="/admin/produkter/rediger/<?= $product['id'] ?>" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition">
                                    Edit
                                </a>
