...
<div class="mb-6">
    <a href="/admin/newsletters" class="inline-flex items-center text-sm text-gray-400 hover:text-white transition">
...
<form method="POST" action="/admin/newsletters/store" id="newsletterForm" class="space-y-6">
    ...
    <div class="bg-gray-800 border border-gray-700 rounded-xl p-6">
        <h3 class="text-lg font-semibold text-white mb-6">Newsletter Content</h3>
        <div class="space-y-6">
            <div>
            ...
            <div>
                <label for="body_html" class="block text-sm font-medium text-gray-300 mb-2">Body *</label>
                <textarea name="body_html" id="body_html" rows="12"
                    class="w-full px-4 py-2.5 bg-gray-700 border border-gray-600 text-white placeholder-gray-400 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"><?= h($newsletter['body_html'] ?? '') ?></textarea>
            </div>
        </div>
    </div>
    ...
    <div class="bg-gray-800 border border-gray-700 rounded-xl p-6">
        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3">
            ...
            <?php if ($newsletter): ?>
            <button type="button" id="sendTestBtn" class="inline-flex items-center px-5 py-2.5 bg-gray-700 hover:bg-gray-600 text-white text-sm font-medium rounded-lg transition border border-gray-600">
...
<div id="confirmModal" class="fixed inset-0 bg-black/60 z-50 hidden items-center justify-center" x-data>
    <div class="bg-gray-800 border border-gray-700 rounded-xl p-6 max-w-md mx-4 w-full">
        <div class="flex items-center mb-4">
            <div class="flex-shrink-0 w-10 h-10 rounded-full bg-yellow-900/50 flex items-center justify-center mr-3">
                <svg class="w-5 h-5 text-yellow-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.34 16.5c-.77.833.192 2.5 1.732 2.5z"/></svg>
            </div>
            <h3 class="text-lg font-semibold text-white">Confirm Send</h3>
        </div>
        <p class="text-gray-300 text-sm mb-6">Are you sure you want to send this newsletter to all subscribers? This action cannot be undone.</p>
        <div class="flex items-center justify-end space-x-3">
            <button type="button" id="cancelSendBtn" class="px-4 py-2 text-sm font-medium text-gray-300 bg-gray-700 hover:bg-gray-600 rounded-lg transition">
                Cancel
            </button>
