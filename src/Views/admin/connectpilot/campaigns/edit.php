        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="description" id="description" rows="3"
                    class="w-full px-4 py-2.5 bg-white border border-gray-300 text-gray-900 placeholder-gray-400 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"><?= h($campaign['description'] ?? '') ?></textarea>
            </div>
            <div>
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div class="bg-gray-50 rounded-lg p-4 text-center">
                <p class="text-2xl font-bold text-gray-900"><?= number_format($campaign['leads_found'] ?? 0) ?></p>
                <p class="text-xs text-gray-500 mt-1">Leads Found</p>
            </div>
            <div class="bg-gray-50 rounded-lg p-4 text-center">
                <p class="text-2xl font-bold text-gray-900"><?= number_format($campaign['connections_sent'] ?? 0) ?></p>
                <p class="text-xs text-gray-500 mt-1">Connections Sent</p>
            </div>
            <div class="bg-gray-50 rounded-lg p-4 text-center">
                <p class="text-2xl font-bold text-gray-900"><?= number_format($campaign['replies_received'] ?? 0) ?></p>
                <p class="text-xs text-gray-500 mt-1">Replied</p>
            </div>
            <div class="bg-gray-50 rounded-lg p-4 text-center">
                <p class="text-2xl font-bold text-gray-900"><?= number_format($campaign['connections_accepted'] ?? 0) ?></p>
...
function campaignForm() {
    return {
        steps: <?php
            $mappedSteps = !empty($steps) ? array_map(function($s) {
                return [
                    'type' => $s['step_type'] ?? 'view_profile',
                    'condition' => $s['step_condition'] ?? 'always',
                    'message_template' => $s['message_template'] ?? '',
                    'delay_days' => (int)($s['delay_days'] ?? 0),
                    'delay_hours' => (int)($s['delay_hours'] ?? 0),
                ];
            }, $steps) : [
                ['type' => 'view_profile', 'condition' => 'always', 'message_template' => '', 'delay_days' => 0, 'delay_hours' => 0],
            ];
            echo json_encode(array_values($mappedSteps));
        ?>,
        addStep() {
            this.steps.push({
                type: 'message',
