...
<div class="mb-8">
    ...
    <div class="bg-gray-800 border border-green-700/50 rounded-xl p-6">
        <div class="flex items-center justify-between">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm font-medium text-green-400">LinkedIn Connected</p>
                    <p class="text-white font-semibold"><?= h($linkedinAccount['linkedin_name']) ?></p>
                    <?php if ($linkedinAccount['linkedin_email']): ?>
                    <p class="text-sm text-gray-400"><?= h($linkedinAccount['linkedin_email']) ?></p>
                    <?php endif; ?>
                </div>
            </div>
    ...
    <?php else: ?>
    <div class="bg-gray-800 border border-red-700/50 rounded-xl p-6">
        <div class="flex items-center justify-between">
            <div class="flex items-center">
                <div class="p-3 bg-red-900/50 rounded-lg">
                    <svg class="w-6 h-6 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.34 16.5c-.77.833.192 2.5 1.732 2.5z"/></svg>
                </div>
                <div class="ml-4">
                    <p class="text-sm font-medium text-red-400">LinkedIn Disconnected</p>
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    <div class="bg-gray-800 rounded-xl p-6 border border-gray-700">
        <div class="flex items-center">
            <div class="p-3 rounded-lg bg-indigo-900/50 text-indigo-400">
            ...
            <div class="ml-4">
                ...
                <p class="text-2xl font-bold text-white"><?= number_format($activeCampaigns) ?></p>
            </div>
        </div>
    </div>
    <div class="bg-gray-800 rounded-xl p-6 border border-gray-700">
    ...
    <div class="bg-gray-800 rounded-xl p-6 border border-gray-700">
        <div class="flex items-center">
            ...
            <div class="ml-4">
                <p class="text-sm text-gray-400">Connections Sent</p>
                <p class="text-2xl font-bold text-white"><?= number_format($totalConnectionsSent) ?></p>
            </div>
        </div>
    </div>
    <div class="bg-gray-800 rounded-xl p-6 border border-gray-700">
        <div class="flex items-center">
            ...
            <div class="ml-4">
                ...
                <p class="text-2xl font-bold text-white"><?= number_format($totalMessagesSent) ?></p>
            </div>
        </div>
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    <div class="bg-gray-800 border border-gray-700 rounded-xl overflow-hidden">
        ...
        <div class="divide-y divide-gray-700">
            ...
            <div class="px-6 py-4 flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-white"><?= h($campaign['name']) ?></p>
    ...
    <div class="bg-gray-800 border border-gray-700 rounded-xl overflow-hidden">
        ...
        <div class="p-8 text-center">
            <p class="text-gray-400">No activity yet. Start a campaign to see activity here.</p>
        </div>
        <?php else: ?>
        <div class="divide-y divide-gray-700 max-h-96 overflow-y-auto">
            ...
            <div class="px-6 py-3 flex items-start">
                <?php
                $typeIcons = [
                    'connection_sent' => ['icon' => 'M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z', 'color' => 'text-blue-400'],
                    'connection_accepted' => ['icon' => 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z', 'color' => 'text-green-400'],
                    'message_sent' => ['icon' => 'M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z', 'color' => 'text-purple-400'],
                    'profile_viewed' => ['icon' => 'M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z', 'color' => 'text-yellow-400'],
                    'reply_received' => ['icon' => 'M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z', 'color' => 'text-indigo-400'],
                ];
                $typeInfo = $typeIcons[$activity['action_type'] ?? ''] ?? ['icon' => 'M13 10V3L4 14h7v7l9-11h-7z', 'color' => 'text-gray-400'];
                ?>
                ...
                <div class="ml-3 min-w-0 flex-1">
                    <p class="text-sm text-gray-300"><?= h($activity['description'] ?? $activity['action_type'] ?? '') ?></p>
