...
<div class="flex items-center justify-between mb-6">
    <div>
        ...
        <p class="text-sm text-gray-500 mt-1"><?= number_format($totalLeads) ?> total leads collected.</p>
    </div>
</div>
...
<div class="bg-white border border-gray-200 rounded-xl shadow-sm p-4 mb-6">
    <form method="GET" action="/admin/connectpilot/leads" class="flex flex-col sm:flex-row items-stretch sm:items-end gap-4">
        ...
        <div class="flex items-end space-x-2">
            ...
            <a href="/admin/connectpilot/leads" class="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-medium rounded-lg transition">
                Clear
            </a>
            <?php endif; ?>
        </div>
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4">
                            <div class="flex items-center">
                                <div class="flex-shrink-0 w-9 h-9 rounded-full bg-gray-100 flex items-center justify-center">
                                    ...
                                    <?php else: ?>
                                    <span class="text-sm font-medium text-gray-500"><?= strtoupper(substr($lead['full_name'] ?? '?', 0, 1)) ?></span>
                                    <?php endif; ?>
                                </div>
                                <div class="ml-3">
                                    <p class="text-sm font-medium text-gray-900"><?= h($lead['full_name']) ?></p>
                                    <?php if ($lead['job_title']): ?>
                                    <p class="text-xs text-gray-500 truncate max-w-xs"><?= h($lead['job_title']) ?></p>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-600"><?= h($lead['company'] ?? '-') ?></td>
                        <td class="px-6 py-4">
                            <?php
                            $leadStatusColors = [
                            ...
                            $leadStatusClass = $leadStatusColors[$lead['status'] ?? 'new'] ?? 'bg-gray-100 text-gray-700';
                            ?>
                        ...
                        <td class="px-6 py-4 text-sm text-gray-500"><?= h($lead['location'] ?? '-') ?></td>
                        <td class="px-6 py-4 text-sm text-gray-500"><?= formatDate($lead['created_at']) ?></td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end space-x-2">
                                <?php if (!empty($lead['linkedin_url'])): ?>
                                <a href="<?= h($lead['linkedin_url']) ?>" target="_blank" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition" title="View on LinkedIn">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg>
                                </a>
                                <?php endif; ?>
                                <?php if ($lead['email']): ?>
                                <a href="mailto:<?= h($lead['email']) ?>" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition" title="Send Email">
        ...
        <div class="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
            ...
            <div class="flex items-center space-x-2">
                ...
                <a href="/admin/connectpilot/leads?page=<?= $page - 1 ?><?= $search ? '&search=' . urlencode($search) : '' ?><?= $campaignId ? '&campaign_id=' . $campaignId : '' ?>"
                    class="px-3 py-1.5 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition">
                    Previous
                </a>
                <?php endif; ?>
