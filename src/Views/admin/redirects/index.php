...
<div class="flex items-center justify-between mb-6">
    <div>
        <h2 class="text-2xl font-bold text-white">Redirects</h2>
        <p class="text-sm text-gray-400 mt-1">Manage URL redirects for old or external links.</p>
    </div>
...
<div class="bg-gray-800 border border-gray-700 rounded-xl overflow-hidden">
    <?php if (empty($redirects)): ?>
        <div class="p-12 text-center">
            ...
            <p class="text-gray-400 mb-4">No redirects yet. Add your first redirect to forward old URLs.</p>
            <a href="/admin/redirects/create" class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg transition">
                Add Redirect
            </a>
        </div>
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-gray-700">
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">From Path</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">To Path</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Status</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Hits</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Last Hit</th>
                ...
                <tbody class="divide-y divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-750" x-data="{ confirmDelete: false }">
                        <td class="px-6 py-4">
                            <div class="text-sm font-medium text-white font-mono"><?= h($r['from_path']) ?></div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="text-sm text-gray-300 font-mono"><?= h($r['to_path']) ?></div>
                        </td>
                        <td class="px-6 py-4">
                            <?php if ((int)$r['status_code'] === 301): ?>
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-900 text-blue-300">301</span>
                            <?php else: ?>
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-900 text-yellow-300">302</span>
                            <?php endif; ?>
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-300"><?= number_format($r['hit_count'] ?? 0) ?></td>
                        <td class="px-6 py-4 text-sm text-gray-300"><?= $r['last_hit_at'] ? formatDate($r['last_hit_at']) : '—' ?></td>
                        <td class="px-6 py-4">
                            <?php if ($r['is_active']): ?>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end space-x-2">
                                <a href="/admin/redirects/edit?id=<?= $r['id'] ?>" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-gray-300 bg-gray-700 hover:bg-gray-600 rounded-lg transition">
                                    Edit
                                </a>
