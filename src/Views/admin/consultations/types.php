...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        <h2 class="text-2xl font-bold text-gray-900">Consultation Types</h2>
        <p class="text-sm text-gray-500 mt-1">Define the types of consultations you offer.</p>
    </div>
    <a href="/admin/consultations" class="inline-flex items-center px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 text-sm font-medium rounded-lg transition">
...
<div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6 mb-8">
    <h3 class="text-lg font-semibold text-gray-900 mb-4">Add New Consultation Type</h3>
...
<div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-200">
        <h3 class="text-lg font-semibold text-gray-900">Existing Types</h3>
    </div>
    <?php if (empty($types)): ?>
        <div class="p-12 text-center">
            <svg class="w-12 h-12 text-gray-600 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/></svg>
            <p class="text-gray-500">No consultation types yet. Create your first type above.</p>
        </div>
    <?php else: ?>
                ...
                <div x-show="!editing" class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div class="flex-1">
                        <div class="flex items-center gap-3">
                            <h4 class="text-base font-medium text-gray-900"><?= h($type['name']) ?></h4>
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium <?= $type['status'] === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500' ?>">
                                <?= ucfirst($type['status']) ?>
                            </span>
                        </div>
                        <?php if ($type['description']): ?>
                            <p class="text-sm text-gray-500 mt-1"><?= h($type['description']) ?></p>
                        <?php endif; ?>
