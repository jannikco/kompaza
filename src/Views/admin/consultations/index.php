...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        <h2 class="text-2xl font-bold text-white">Consultation Bookings</h2>
        <p class="text-sm text-gray-400 mt-1">Manage incoming consultation requests from customers.</p>
    </div>
...
<div class="mb-6 flex flex-wrap gap-2">
    <a href="/admin/consultations"
       class="inline-flex items-center px-4 py-2 text-sm font-medium rounded-lg transition border <?= empty($statusFilter) ? 'bg-indigo-600 border-indigo-500 text-white' : 'bg-gray-800 border-gray-700 text-gray-300 hover:bg-gray-700 hover:text-white' ?>">
        All
        <span class="ml-2 px-2 py-0.5 text-xs rounded-full <?= empty($statusFilter) ? 'bg-indigo-500 text-white' : 'bg-gray-700 text-gray-400' ?>"><?= $counts['all'] ?></span>
    </a>
    ...
    <a href="/admin/consultations?status=<?= $status ?>"
       class="inline-flex items-center px-4 py-2 text-sm font-medium rounded-lg transition border <?= ($statusFilter ?? '') === $status ? 'bg-indigo-600 border-indigo-500 text-white' : 'bg-gray-800 border-gray-700 text-gray-300 hover:bg-gray-700 hover:text-white' ?>">
        <?= ucfirst($status) ?>
        <span class="ml-2 px-2 py-0.5 text-xs rounded-full <?= ($statusFilter ?? '') === $status ? 'bg-indigo-500 text-white' : 'bg-gray-700 text-gray-400' ?>"><?= $counts[$status] ?></span>
    </a>
    <?php endforeach; ?>
</div>
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-gray-700 bg-gray-800/50">
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Booking #</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Customer</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Type</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Preferred Date</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Urgency</th>
                ...
                <tbody class="divide-y divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-750 transition-colors" x-data="{ open: false }">
                        <td class="px-6 py-4">
                            <span class="text-sm font-medium text-indigo-400"><?= h($booking['booking_number']) ?></span>
                        </td>
                        <td class="px-6 py-4">
                            <div class="text-sm font-medium text-white"><?= h($booking['customer_name']) ?></div>
                            <div class="text-xs text-gray-400"><?= h($booking['customer_email']) ?></div>
                            <?php if (!empty($booking['company'])): ?>
                                <div class="text-xs text-gray-500"><?= h($booking['company']) ?></div>
                            <?php endif; ?>
                        </td>
                        <td class="px-6 py-4">
                            <span class="text-sm text-gray-300"><?= h($booking['type_name'] ?? 'N/A') ?></span>
                        ...
                        <td class="px-6 py-4">
                            ...
                            <div class="text-xs text-gray-400"><?= h($timeLabels[$booking['preferred_time']] ?? $booking['preferred_time'] ?? '') ?></div>
                        </td>
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium <?= $urgencyBadgeColors[$booking['urgency']] ?? 'bg-gray-700 text-gray-300' ?>">
                                <?= ucfirst($booking['urgency'] ?? 'medium') ?>
                            </span>
                        </td>
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium <?= $statusBadgeColors[$booking['status']] ?? 'bg-gray-700 text-gray-300' ?>">
                        ...
                        <td class="px-6 py-4 text-right">
                            <button @click="open = !open" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-gray-300 bg-gray-700 hover:bg-gray-600 rounded-lg transition">
                    ...
                    <tr x-show="open" x-cloak class="bg-gray-800/50">
                        <td colspan="8" class="px-6 py-4">
                            <form method="POST" action="/admin/consultations/update-status" class="flex flex-col md:flex-row items-start md:items-end gap-4">
                                ...
                                <div class="w-full md:w-auto flex-shrink-0">
                                    <label class="block text-xs font-medium text-gray-400 mb-1">Project Description</label>
                                    <p class="text-sm text-gray-300 bg-gray-700 rounded-lg p-3 max-w-md"><?= h($booking['project_description']) ?></p>
                                </div>
                                <?php endif; ?>
                                ...
                                <div class="flex-shrink-0">
                                    ...
                                    <p class="text-sm text-gray-300"><?= h($booking['customer_phone']) ?></p>
                                </div>
                                <?php endif; ?>

                                <div class="flex-shrink-0">
                                    <label class="block text-xs font-medium text-gray-400 mb-1">Status</label>
                                    <select name="status" class="px-3 py-2 bg-gray-700 border border-gray-600 text-white rounded-lg text-sm focus:ring-2 focus:ring-indigo-500">
                                ...
                                <div class="flex gap-2 flex-shrink-0">
                                    ...
                                    <button type="button" @click="open = false" class="px-4 py-2 bg-gray-700 hover:bg-gray-600 text-gray-300 text-sm font-medium rounded-lg transition">
                                        Cancel
                                    </button>
                                </div>
                            </form>
