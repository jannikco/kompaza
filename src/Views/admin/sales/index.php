    ...
    <div class="bg-white rounded-xl p-6 border border-gray-200 shadow-sm">
        <div class="flex items-center">
            ...
            <div class="ml-4">
                ...
                <p class="text-2xl font-bold text-gray-900"><?= number_format($emailSignupCount) ?></p>
            </div>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mb-8">
    <h2 class="text-lg font-semibold text-gray-900 mb-4">Revenue - Last 30 Days</h2>
    <?php if ($maxRevenue > 0): ?>
...
<!-- Tabbed sections -->
<div x-data="{ activeTab: 'orders' }" class="bg-white rounded-xl border border-gray-200 shadow-sm">
    <!-- Tab navigation -->
    <div class="border-b border-gray-200 px-6">
        <nav class="flex -mb-px space-x-6 overflow-x-auto">
            <button @click="activeTab = 'orders'"
                :class="activeTab === 'orders' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-600 hover:border-gray-300'"
                class="py-3 px-1 border-b-2 text-sm font-medium whitespace-nowrap transition-colors">
                Orders
            </button>
            <button @click="activeTab = 'ebooks'"
                :class="activeTab === 'ebooks' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-600 hover:border-gray-300'"
                class="py-3 px-1 border-b-2 text-sm font-medium whitespace-nowrap transition-colors">
                Ebook Sales
            </button>
            <?php if (tenantFeature('courses')): ?>
            <button @click="activeTab = 'courses'"
                :class="activeTab === 'courses' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-600 hover:border-gray-300'"
                class="py-3 px-1 border-b-2 text-sm font-medium whitespace-nowrap transition-colors">
                Courses
            </button>
            <button @click="activeTab = 'certificates'"
                :class="activeTab === 'certificates' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-600 hover:border-gray-300'"
                class="py-3 px-1 border-b-2 text-sm font-medium whitespace-nowrap transition-colors">
                Certificates
            </button>
            <?php endif; ?>
            ...
            <button @click="activeTab = 'consultations'"
                :class="activeTab === 'consultations' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-600 hover:border-gray-300'"
                class="py-3 px-1 border-b-2 text-sm font-medium whitespace-nowrap transition-colors">
                Consultations
            </button>
            <?php endif; ?>
    ...
    <div x-show="activeTab === 'orders'" x-cloak>
        <div class="px-6 py-4 flex items-center justify-between border-b border-gray-200/50">
            ...
            <table class="w-full">
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        ...
                        <tr class="border-b border-gray-200/50 hover:bg-gray-100/30">
                            <td class="px-6 py-3 text-sm text-white font-medium"><?= h($order['order_number'] ?? '-') ?></td>
                            <td class="px-6 py-3 text-sm text-gray-600"><?= h($order['customer_name'] ?? '-') ?></td>
                            ...
                            <td class="px-6 py-3 text-sm text-gray-500"><?= formatDate($order['created_at']) ?></td>
                        </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </tbody>
    ...
    <div x-show="activeTab === 'ebooks'" x-cloak>
        <div class="px-6 py-4 flex items-center justify-between border-b border-gray-200/50">
            <h3 class="text-base font-semibold text-gray-900">Ebook Purchases (<?= $ebookSalesCount ?> total, <?= formatMoney($ebookRevenue / 100) ?> revenue)</h3>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead>
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        ...
                        <tr class="border-b border-gray-200/50 hover:bg-gray-100/30">
                            <td class="px-6 py-3 text-sm text-gray-600"><?= formatDate($purchase['created_at']) ?></td>
                            <td class="px-6 py-3 text-sm text-white"><?= h($purchase['ebook_title'] ?? '-') ?></td>
                            <td class="px-6 py-3 text-sm text-gray-600"><?= h($purchase['customer_email'] ?? '-') ?></td>
                            <td class="px-6 py-3 text-sm text-white"><?= formatMoney(($purchase['amount_cents'] ?? 0) / 100) ?></td>
                            <td class="px-6 py-3 text-sm text-gray-500"><?= formatMoney(($purchase['application_fee_cents'] ?? 0) / 100) ?></td>
                            <td class="px-6 py-3">
                                <?php
                                ...
                                $purchaseColor = $purchaseStatusColors[$purchase['status']] ?? 'bg-gray-100 text-gray-600';
                                ?>
    ...
    <div x-show="activeTab === 'courses'" x-cloak>
        <div class="px-6 py-4 border-b border-gray-200/50">
            <h3 class="text-base font-semibold text-gray-900">Course Statistics</h3>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead>
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        ...
                        <tr class="border-b border-gray-200/50 hover:bg-gray-100/30">
                            ...
                            <td class="px-6 py-3 text-sm text-gray-600 text-center"><?= number_format($cs['enrolled']) ?></td>
                            <td class="px-6 py-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <div class="w-20 bg-gray-200 rounded-full h-2">
                                        <div class="bg-indigo-500 h-2 rounded-full" style="width: <?= min($cs['completion_rate'], 100) ?>%"></div>
                                    </div>
                                    <span class="text-sm text-gray-600"><?= $cs['completion_rate'] ?>%</span>
                                </div>
                            </td>
                            <td class="px-6 py-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <span class="text-sm text-gray-600"><?= $cs['quiz_pass_rate'] ?>%</span>
                                </div>
                            </td>
                            <td class="px-6 py-3 text-sm text-gray-600 text-center"><?= number_format($cs['certificates']) ?></td>
                        </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </tbody>
    ...
    <div x-show="activeTab === 'certificates'" x-cloak>
        <div class="px-6 py-4 flex items-center justify-between border-b border-gray-200/50">
            <h3 class="text-base font-semibold text-gray-900">Certificates Issued: <?= number_format($certificateCount) ?></h3>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead>
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        ...
                        <tr class="border-b border-gray-200/50 hover:bg-gray-100/30">
                            ...
                            <td class="px-6 py-3 text-sm text-gray-600"><?= h($cert['user_name'] ?? '-') ?></td>
                            ...
                            <td class="px-6 py-3 text-sm text-gray-500"><?= formatDate($cert['issued_at'] ?? $cert['created_at'] ?? '') ?></td>
                        </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </tbody>
    ...
    <div x-show="activeTab === 'consultations'" x-cloak>
        <div class="px-6 py-4 border-b border-gray-200/50">
            <h3 class="text-base font-semibold text-gray-900">Consultation Bookings: <?= number_format($consultationCount) ?></h3>
        </div>
        <div class="p-6">
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div class="bg-gray-50 rounded-lg p-5 border border-gray-200">
                    ...
                    <p class="text-3xl font-bold text-yellow-600"><?= number_format($consultationPending) ?></p>
                </div>
                <!-- Confirmed -->
                <div class="bg-gray-50 rounded-lg p-5 border border-gray-200">
                    <div class="flex items-center justify-between mb-2">
                    ...
                    <p class="text-3xl font-bold text-blue-600"><?= number_format($consultationConfirmed) ?></p>
                </div>
                <!-- Completed -->
                <div class="bg-gray-50 rounded-lg p-5 border border-gray-200">
                    <div class="flex items-center justify-between mb-2">
                    ...
                    <p class="text-3xl font-bold text-green-600"><?= number_format($consultationCompleted) ?></p>
                </div>
            </div>
