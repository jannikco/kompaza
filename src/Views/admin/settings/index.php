...
<form method="POST" action="/admin/indstillinger/opdater" enctype="multipart/form-data" class="space-y-8">
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
        ...
        <p class="text-sm text-gray-500 mb-4">Choose your template, customize hero buttons, and arrange content sections.</p>
        <a href="/admin/homepage" class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg transition">
        ...
        <!-- Hero Subtitle (kept here since it's general branding) -->
        <div class="mt-6">
        ...
        <div class="mt-6">
            ...
            <?php if (!empty($tenant['hero_image_path'])): ?>
                <div class="mb-3 flex items-center space-x-3">
                    <img src="<?= h(imageUrl($tenant['hero_image_path'])) ?>" alt="Current hero image" class="h-20 w-auto rounded border border-gray-300">
                    <span class="text-sm text-gray-500">Current hero image</span>
                </div>
            <?php endif; ?>
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-6 flex items-center">
            <svg class="w-5 h-5 mr-2 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
            Contact Information
        </h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
            ...
            <div>
                <label for="cvr_number" class="block text-sm font-medium text-gray-700 mb-2">CVR Number</label>
    ...
    <!-- Integrations -->
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6" x-data="{ emailService: '<?= h($tenant['email_service'] ?? 'kompaza') ?>' }">
        <h3 class="text-lg font-semibold text-gray-900 mb-6 flex items-center">
            <svg class="w-5 h-5 mr-2 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"/></svg>
            Integrations
        </h3>

        <!-- Email Service Provider -->
        <div class="mb-6">
            <label for="email_service" class="block text-sm font-medium text-gray-700 mb-2">Email Service Provider</label>
        ...
        <div x-show="emailService === 'kompaza'" x-cloak class="mb-6">
            <p class="text-sm text-gray-500">No configuration needed. Emails are sent via the Kompaza platform.</p>
        </div>

        <!-- Brevo fields -->
        <div x-show="emailService === 'brevo'" x-cloak class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                <label for="brevo_api_key" class="block text-sm font-medium text-gray-700 mb-2">Brevo API Key</label>
            ...
            <div>
                <label for="brevo_list_id" class="block text-sm font-medium text-gray-700 mb-2">Brevo List ID</label>
        ...
        <div x-show="emailService === 'mailgun'" x-cloak class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                <label for="mailgun_api_key" class="block text-sm font-medium text-gray-700 mb-2">Mailgun API Key</label>
            ...
            <div class="md:col-span-2">
                <p class="text-sm text-yellow-600">Contact list sync is only available with Brevo. Newsletter signups will be recorded locally but not synced to an external list.</p>
            </div>
        </div>
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
        ...
        <div class="space-y-6">
            ...
            <div>
                <label for="custom_footer_html" class="block text-sm font-medium text-gray-700 mb-2">Custom Footer HTML</label>
