        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="short_description" id="short_description" rows="2"
                    class="w-full px-4 py-2.5 bg-gray-700 border border-gray-600 text-white placeholder-gray-400 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"><?= h($program['short_description'] ?? '') ?></textarea>
            </div>
            <div class="md:col-span-2">
...
<div class="bg-gray-800 border border-gray-700 rounded-xl p-6 mb-8">
    <div class="flex items-center justify-between mb-6">
        <h3 class="text-lg font-semibold text-white">Tiers</h3>
        <span class="text-sm text-gray-400"><?= count($tiers) ?> tier<?= count($tiers) !== 1 ? 's' : '' ?></span>
    </div>

    <?php if (!empty($tiers)): ?>
    <div class="space-y-4 mb-6">
        <?php foreach ($tiers as $tier): ?>
        <div class="bg-gray-900 border border-gray-700 rounded-lg p-5" x-data="{ editing: false, confirmDelete: false }">
            <!-- View mode -->
            <div x-show="!editing">
                <div class="flex items-start justify-between">
                    <div>
                        ...
                        <p class="text-sm text-gray-400 mt-1"><?= h($tier['description']) ?></p>
                        <?php endif; ?>
                        ...
                        <div class="mt-2">
                            <?php foreach (explode("\n", $tier['features']) as $feature): ?>
                                <?php if (trim($feature)): ?>
                                <span class="inline-block text-xs text-gray-400 bg-gray-800 px-2 py-0.5 rounded mr-1 mb-1"><?= h(trim($feature)) ?></span>
                                <?php endif; ?>
                            <?php endforeach; ?>
                        </div>
                        <?php endif; ?>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="name" required value="<?= h($tier['name']) ?>"
                                class="w-full px-3 py-2 bg-gray-700 border border-gray-600 text-white rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-sm">
                        </div>
                        <div>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-400 mb-1">Upfront Price (DKK)</label>
                            <input type="number" name="upfront_price_dkk" step="0.01" min="0" value="<?= $tier['upfront_price_dkk'] ?>"
                                class="w-full px-3 py-2 bg-gray-700 border border-gray-600 text-white rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-400 mb-1">Monthly Price (DKK)</label>
                            <input type="number" name="monthly_price_dkk" step="0.01" min="0" value="<?= $tier['monthly_price_dkk'] ?>"
                                class="w-full px-3 py-2 bg-gray-700 border border-gray-600 text-white rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-sm">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-400 mb-1">Max Members</label>
                            <input type="number" name="max_members" min="0" value="<?= $tier['max_members'] ?? '' ?>"
                                class="w-full px-3 py-2 bg-gray-700 border border-gray-600 text-white rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-sm"
                                placeholder="Unlimited">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-400 mb-1">Features (one per line)</label>
                            <textarea name="features" rows="3"
                                class="w-full px-3 py-2 bg-gray-700 border border-gray-600 text-white rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-sm"><?= h($tier['features'] ?? '') ?></textarea>
                        </div>
                    </div>
                    <div class="flex items-center space-x-2">
                        ...
                        <button type="button" @click="editing = false" class="px-4 py-2 text-sm font-medium text-gray-300 bg-gray-700 hover:bg-gray-600 rounded-lg transition">
                            Cancel
                        </button>
                    </div>
                </form>
    ...
    <div class="border-t border-gray-700 pt-6">
        <h4 class="text-sm font-semibold text-gray-300 mb-4">Add New Tier</h4>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="name" required
                        class="w-full px-3 py-2 bg-gray-700 border border-gray-600 text-white placeholder-gray-400 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-sm"
...
<div class="bg-gray-800 border border-gray-700 rounded-xl p-6">
    <div class="flex items-center justify-between mb-6">
        ...
        <span class="text-sm text-gray-400"><?= count($enrollments) ?> enrollment<?= count($enrollments) !== 1 ? 's' : '' ?></span>
    </div>
        ...
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-700">
                ...
                <tr>
                    ...
                    <td class="px-4 py-3 text-sm text-gray-300"><?= h($enrollment['tier_name'] ?? 'N/A') ?></td>
                    <td class="px-4 py-3">
                        <?php
                        $enrollStatusColors = [
                    ...
                    <td class="px-4 py-3 text-sm text-gray-400"><?= formatDate($enrollment['enrolled_at'] ?? $enrollment['created_at'] ?? '') ?></td>
                    <td class="px-4 py-3 text-right">
                        <div class="flex items-center justify-end space-x-1">
                            ...
                            <form method="POST" action="/admin/mastermind/enrollment-update" class="inline">
                                ...
                                <button type="submit" class="px-2 py-1 text-xs font-medium text-yellow-400 bg-gray-700 hover:bg-yellow-900/50 rounded transition" title="Pause">
                                    Pause
                                </button>
                            </form>
                            <form method="POST" action="/admin/mastermind/enrollment-update" class="inline">
                                ...
                                <button type="submit" class="px-2 py-1 text-xs font-medium text-blue-400 bg-gray-700 hover:bg-blue-900/50 rounded transition" title="Complete">
                                    Complete
                                </button>
                            </form>
                            <form method="POST" action="/admin/mastermind/enrollment-update" class="inline">
                                ...
                                <button type="submit" class="px-2 py-1 text-xs font-medium text-red-400 bg-gray-700 hover:bg-red-900/50 rounded transition" title="Cancel">
                                    Cancel
                                </button>
                            </form>
                            ...
                            <form method="POST" action="/admin/mastermind/enrollment-update" class="inline">
                                ...
                                <button type="submit" class="px-2 py-1 text-xs font-medium text-green-400 bg-gray-700 hover:bg-green-900/50 rounded transition" title="Resume">
                                    Resume
                                </button>
                            </form>
                            <form method="POST" action="/admin/mastermind/enrollment-update" class="inline">
                            ...
                            <form method="POST" action="/admin/mastermind/enrollment-update" class="inline">
                                ...
                                <input type="hidden" name="program_id" value="<?= $program['id'] ?>">
                                <input type="hidden" name="status" value="active">
                                <button type="submit" class="px-2 py-1 text-xs font-medium text-green-400 bg-gray-700 hover:bg-green-900/50 rounded transition" title="Reactivate">
                                    Reactivate
                                </button>
                            </form>
                            <?php endif; ?>
