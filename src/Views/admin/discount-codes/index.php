...
<div class="flex items-center justify-between mb-6">
    <div>
        ...
        <p class="text-sm text-gray-500 mt-1">Manage discount codes for your store.</p>
    </div>
...
<div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
    <?php if (empty($discountCodes)): ?>
        <div class="p-12 text-center">
            ...
            <p class="text-gray-500 mb-4">No discount codes yet. Create your first discount code to offer promotions.</p>
            <a href="/admin/discount-codes/create" class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg transition">
                Create Discount Code
            </a>
        </div>
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50" x-data="{ confirmDelete: false }">
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center px-2.5 py-1 rounded bg-gray-100 text-sm font-mono font-semibold text-gray-900 tracking-wide"><?= h($dc['code']) ?></span>
                        </td>
                        <td class="px-6 py-4">
                            <?php if ($dc['type'] === 'percentage'): ?>
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-700">Percentage</span>
                            <?php else: ?>
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-700">Fixed</span>
                            <?php endif; ?>
                        </td>
                        ...
                        <td class="px-6 py-4 text-sm text-gray-500">
                            <?php if ($dc['expires_at']): ?>
                                ...
                                ?>
                                <span class="<?= $isExpired ? 'text-red-600' : '' ?>"><?= formatDate($dc['expires_at'], 'd M Y H:i') ?></span>
                            <?php else: ?>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end space-x-2">
                                <a href="/admin/discount-codes/edit?id=<?= $dc['id'] ?>" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition">
                                    Edit
                                </a>
