...
<div class="mb-6">
    ...
    <h2 class="text-2xl font-bold text-white mt-1">Create Course</h2>
    <p class="text-sm text-gray-400 mt-1">Set up your new course. You can add modules and lessons after creating.</p>
</div>
...
<form method="POST" action="/admin/kurser/gem" enctype="multipart/form-data" class="max-w-4xl" x-data="{ pricingType: '<?= h($_POST['pricing_type'] ?? 'free') ?>' }">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <textarea id="editor" name="description" rows="10"
                          class="w-full px-4 py-3 bg-gray-700 border border-gray-600 text-white placeholder-gray-400 rounded-lg text-sm"><?= h($_POST['description'] ?? '') ?></textarea>
            </div>
        </div>
    </div>
    ...
    <div class="bg-gray-800 border border-gray-700 rounded-xl p-6 mb-6">
        ...
        <div>
            <label for="cover_image" class="block text-sm font-medium text-gray-300 mb-1.5">Course Thumbnail</label>
            <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-600 border-dashed rounded-lg hover:border-gray-500 transition">
                <div class="space-y-1 text-center">
                    ...
                    <div class="text-sm text-gray-400">
                        <label for="cover_image" class="relative cursor-pointer text-indigo-400 hover:text-indigo-300 font-medium">
    ...
    <div class="bg-gray-800 border border-gray-700 rounded-xl p-6 mb-6">
        ...
        <div class="space-y-4">
            <div>
                <label class="block text-sm font-medium text-gray-300 mb-2">Pricing Type</label>
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    ...
                    <label class="flex items-center p-3 bg-gray-700 border border-gray-600 rounded-lg cursor-pointer hover:border-indigo-500 transition" :class="pricingType === 'one_time' ? 'border-indigo-500 ring-1 ring-indigo-500' : ''">
                        ...
                        <span class="text-sm font-medium text-white">One-Time Purchase</span>
                    </label>
            ...
            <div x-show="pricingType === 'one_time'" x-cloak class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="price_dkk" class="block text-sm font-medium text-gray-300 mb-1.5">Price (DKK)</label>
                    <div class="relative">
                        <input type="number" id="price_dkk" name="price_dkk" step="0.01" min="0" value="<?= h($_POST['price_dkk'] ?? '') ?>"
                               class="w-full pl-4 pr-16 py-2.5 bg-gray-700 border border-gray-600 text-white rounded-lg focus:ring-2 focus:ring-indigo-500 text-sm" placeholder="0.00">
                        <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none"><span class="text-gray-400 text-sm">DKK</span></div>
                    </div>
                </div>
                <div>
            ...
            <div x-show="pricingType === 'subscription'" x-cloak class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-sm font-medium text-gray-300 mb-1.5">Monthly Price (DKK)</label>
                    <input type="number" name="subscription_price_monthly_dkk" step="0.01" min="0" value="<?= h($_POST['subscription_price_monthly_dkk'] ?? '') ?>"
                           class="w-full px-4 py-2.5 bg-gray-700 border border-gray-600 text-white rounded-lg focus:ring-2 focus:ring-indigo-500 text-sm" placeholder="0.00">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-300 mb-1.5">Yearly Price (DKK)</label>
    ...
    <div class="bg-gray-800 border border-gray-700 rounded-xl p-6 mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label for="instructor_name" class="block text-sm font-medium text-gray-300 mb-1.5">Instructor Name</label>
                <input type="text" id="instructor_name" name="instructor_name" value="<?= h($_POST['instructor_name'] ?? '') ?>"
                       class="w-full px-4 py-2.5 bg-gray-700 border border-gray-600 text-white rounded-lg focus:ring-2 focus:ring-indigo-500 text-sm" placeholder="John Doe">
            </div>
            <div>
                <label for="instructor_image" class="block text-sm font-medium text-gray-300 mb-1.5">Instructor Photo</label>
            ...
            <div class="md:col-span-2">
                <label for="instructor_bio" class="block text-sm font-medium text-gray-300 mb-1.5">Instructor Bio</label>
                <textarea id="instructor_bio" name="instructor_bio" rows="3"
                          class="w-full px-4 py-3 bg-gray-700 border border-gray-600 text-white rounded-lg focus:ring-2 focus:ring-indigo-500 text-sm" placeholder="Brief bio..."><?= h($_POST['instructor_bio'] ?? '') ?></textarea>
            </div>
        </div>
    </div>
    ...
    <div class="bg-gray-800 border border-gray-700 rounded-xl p-6 mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <select id="status" name="status" class="w-full px-4 py-2.5 bg-gray-700 border border-gray-600 text-white rounded-lg focus:ring-2 focus:ring-indigo-500 text-sm">
            ...
            <div class="flex items-end space-x-6">
                <label class="flex items-center">
                    <input type="checkbox" name="is_featured" value="1" class="w-4 h-4 text-indigo-600 bg-gray-700 border-gray-600 rounded focus:ring-indigo-500">
                    <span class="ml-2 text-sm text-gray-300">Featured</span>
                </label>
                <label class="flex items-center">
                    <input type="checkbox" name="drip_enabled" value="1" class="w-4 h-4 text-indigo-600 bg-gray-700 border-gray-600 rounded focus:ring-indigo-500">
                    <span class="ml-2 text-sm text-gray-300">Enable drip content</span>
                </label>
            </div>
        </div>
    </div>
