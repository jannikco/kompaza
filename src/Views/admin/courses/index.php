...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        ...
        <p class="text-sm text-gray-400 mt-1">Create and manage your online courses.</p>
    </div>
...
<div class="bg-gray-800 border border-gray-700 rounded-xl overflow-hidden">
    <?php if (empty($courses)): ?>
        <div class="p-12 text-center">
            ...
            <p class="text-gray-400 mb-4">No courses yet. Create your first course to start teaching.</p>
            <a href="/admin/kurser/opret" class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg transition">
                Add Course
            </a>
        </div>
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-gray-700 bg-gray-800/50">
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Pricing</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Lessons</th>
                    ...
                    <tr class="hover:bg-gray-750 transition-colors" x-data="{ confirmDelete: false }">
                        <td class="px-6 py-4">
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    <div class="text-sm font-medium text-white"><?= h($course['title']) ?></div>
                                    <?php if ($course['is_featured']): ?>
                                        <span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-yellow-900/50 text-yellow-300 mt-1">Featured</span>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <?php if ($course['pricing_type'] === 'free'): ?>
                                <span class="text-sm text-green-400 font-medium">Free</span>
                        ...
                        <td class="px-6 py-4 text-sm text-gray-300"><?= (int)$course['total_lessons'] ?></td>
                        <td class="px-6 py-4 text-sm text-gray-300"><?= (int)$course['enrollment_count'] ?></td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end space-x-2">
                                <a href="/admin/kurser/rediger?id=<?= $course['id'] ?>" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-gray-300 bg-gray-700 hover:bg-gray-600 rounded-lg transition">
                                    Edit
                                </a>
                                ...
                                <template x-if="confirmDelete">
                                    <form method="POST" action="/admin/kurser/slet" class="inline-flex items-center space-x-1">
                                        ...
                                        <button type="button" @click="confirmDelete = false" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-gray-300 bg-gray-700 hover:bg-gray-600 rounded-lg transition">Cancel</button>
                                    </form>
                                </template>
                            </div>
                        </td>
