...
<div class="mb-6">
    ...
    <h2 class="text-2xl font-bold text-white mt-1"><?= $isEdit ? 'Edit Quiz' : 'Create Quiz' ?></h2>
    <p class="text-sm text-gray-400">Course: <?= h($course['title']) ?></p>
</div>
...
<form method="POST" action="<?= $isEdit ? '/admin/kurser/quiz/opdater' : '/admin/kurser/quiz/gem' ?>" class="max-w-3xl">
    ...
    <div class="bg-gray-800 border border-gray-700 rounded-xl p-6 mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                <label for="module_id" class="block text-sm font-medium text-gray-300 mb-1.5">Module (optional)</label>
                <select id="module_id" name="module_id" class="w-full px-4 py-2.5 bg-gray-700 border border-gray-600 text-white rounded-lg focus:ring-2 focus:ring-indigo-500 text-sm">
            ...
            <div>
                <label for="pass_threshold" class="block text-sm font-medium text-gray-300 mb-1.5">Pass Threshold (%)</label>
            ...
            <div class="flex items-center">
                <label class="flex items-center cursor-pointer">
                    ...
                    <span class="ml-2 text-sm text-gray-300">Shuffle questions</span>
                </label>
            </div>
        </div>
        <div class="mt-6">
...
<div class="max-w-3xl">
    <div class="flex items-center justify-between mb-4">
        <h3 class="text-lg font-semibold text-white">Questions (<?= count($quiz['questions'] ?? []) ?>)</h3>
    </div>

    <?php if (!empty($quiz['questions'])): ?>
        <?php foreach ($quiz['questions'] as $qIndex => $question): ?>
        <div class="bg-gray-800 border border-gray-700 rounded-xl p-6 mb-4" x-data="{ editing: false }">
            <div class="flex items-start justify-between">
                <div class="flex-1">
                    <p class="text-sm text-gray-400 mb-1">Question <?= $qIndex + 1 ?></p>
                    <p class="text-white font-medium" x-show="!editing"><?= h($question['text']) ?></p>
                </div>
                <div class="flex items-center space-x-2 ml-4">
                    <button @click="editing = !editing" class="text-gray-400 hover:text-white text-sm">
            ...
            <div class="mt-3 space-y-2" x-show="!editing">
                ...
                <div class="flex items-center text-sm <?= $choice['is_correct'] ? 'text-green-400' : 'text-gray-400' ?>">
            ...
            <form method="POST" action="/admin/kurser/quiz/spoergsmaal/opdater" x-show="editing" x-cloak class="mt-4">
                ...
                <input type="hidden" name="quiz_id" value="<?= $quiz['id'] ?>">
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-300 mb-1.5">Question Text</label>
                    <input type="text" name="question_text" value="<?= h($question['text']) ?>" required
                           class="w-full px-4 py-2.5 bg-gray-700 border border-gray-600 text-white rounded-lg focus:ring-2 focus:ring-indigo-500 text-sm">
                </div>
                <div class="space-y-2 mb-4">
                    <label class="block text-sm font-medium text-gray-300">Choices (select the correct answer)</label>
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        <input type="text" name="choices[]" value="<?= h($choice['text']) ?>"
                               class="flex-1 px-3 py-2 bg-gray-700 border border-gray-600 text-white rounded-lg focus:ring-2 focus:ring-indigo-500 text-sm">
                    </div>
                    <?php endforeach; ?>
        ...
        <?php endforeach; ?>
    <?php else: ?>
        <div class="bg-gray-800 border border-gray-700 rounded-xl p-8 text-center mb-4">
            <p class="text-gray-400">No questions yet. Add your first question below.</p>
        </div>
    <?php endif; ?>

    <!-- Add Question Form -->
    <div class="bg-gray-800 border border-gray-700 rounded-xl p-6" x-data="{ open: false }">
        <button @click="open = !open" class="flex items-center text-indigo-400 hover:text-indigo-300 text-sm font-medium">
        ...
        <form method="POST" action="/admin/kurser/quiz/spoergsmaal/gem" x-show="open" x-cloak class="mt-4">
            ...
            <input type="hidden" name="quiz_id" value="<?= $quiz['id'] ?>">
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-300 mb-1.5">Question Text <span class="text-red-400">*</span></label>
