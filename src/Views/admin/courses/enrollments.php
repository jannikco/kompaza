...
<div class="mb-6">
    ...
    <h2 class="text-2xl font-bold text-white mt-1">Students — <?= h($course['title']) ?></h2>
    <p class="text-sm text-gray-400 mt-1"><?= count($enrollments) ?> enrolled students</p>
</div>

<!-- Manual Enrollment -->
<div class="bg-gray-800 border border-gray-700 rounded-xl p-5 mb-6 max-w-xl" x-data="{ showForm: false }">
    ...
    <form x-show="showForm" x-cloak method="POST" action="/admin/kurser/tilmeld" class="flex items-end gap-3">
        ...
        <div class="flex-1">
            <label class="block text-xs font-medium text-gray-400 mb-1">Customer User ID</label>
        ...
        <button type="button" @click="showForm = false" class="px-3 py-2 text-sm text-gray-400 hover:text-white transition">Cancel</button>
    </form>
</div>
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-gray-700 bg-gray-800/50">
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Enrolled</th>
                ...
                <tbody class="divide-y divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-750 transition-colors" x-data="{ confirmUnenroll: false }">
                        <td class="px-6 py-4">
                            <div class="text-sm font-medium text-white"><?= h($enrollment['user_name']) ?></div>
                            <div class="text-xs text-gray-400"><?= h($enrollment['user_email']) ?></div>
                        </td>
                        <td class="px-6 py-4">
                            <?php
                            $sourceColors = [
                                'free' => 'bg-green-900/50 text-green-300',
                                'purchase' => 'bg-blue-900/50 text-blue-300',
                                'subscription' => 'bg-purple-900/50 text-purple-300',
                                'manual' => 'bg-gray-700 text-gray-300',
                            ];
                            $srcColor = $sourceColors[$enrollment['enrollment_source']] ?? 'bg-gray-700 text-gray-300';
                            ?>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center space-x-2">
                                ...
                                <span class="text-xs text-gray-400"><?= (int)$enrollment['completed_lessons'] ?>/<?= (int)$enrollment['total_lessons'] ?></span>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-300"><?= formatDate($enrollment['enrolled_at']) ?></td>
                        <td class="px-6 py-4">
                            ...
                            <?php else: ?>
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-700 text-gray-300"><?= h(ucfirst($enrollment['status'])) ?></span>
                            <?php endif; ?>
                        </td>
                        <td class="px-6 py-4 text-right">
                            <?php if ($enrollment['status'] === 'active'): ?>
                            <template x-if="!confirmUnenroll">
                                <button @click="confirmUnenroll = true" class="text-xs text-red-400 hover:text-red-300 transition">Unenroll</button>
                            </template>
                            <template x-if="confirmUnenroll">
                                <form method="POST" action="/admin/kurser/afmeld" class="inline-flex items-center space-x-1">
                                    ...
                                    <button type="button" @click="confirmUnenroll = false" class="text-xs text-gray-400 px-2 py-1">Cancel</button>
                                </form>
                            </template>
                            <?php endif; ?>
                        </td>
