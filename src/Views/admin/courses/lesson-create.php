...
<div class="mb-6">
    ...
    <h2 class="text-2xl font-bold text-white mt-1">Add Lesson</h2>
...
<form method="POST" action="/admin/kurser/lektion/gem" class="max-w-4xl" x-data="{ lessonType: '<?= h($_POST['lesson_type'] ?? 'video') ?>' }">
    ...
    <div x-show="lessonType !== 'video'" x-cloak class="bg-gray-800 border border-gray-700 rounded-xl p-6 mb-6">
        ...
        <textarea id="lesson_editor" name="text_content" rows="15"
                  class="w-full bg-gray-700 border border-gray-600 text-white rounded-lg text-sm"><?= h($_POST['text_content'] ?? '') ?></textarea>
    </div>
    ...
    <div class="bg-gray-800 border border-gray-700 rounded-xl p-6 mb-6">
        ...
        <div class="space-y-3">
            <label class="flex items-center">
                <input type="checkbox" name="is_preview" value="1" class="w-4 h-4 text-indigo-600 bg-gray-700 border-gray-600 rounded focus:ring-indigo-500">
