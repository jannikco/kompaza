...
<div class="mb-6">
    ...
    <div class="flex items-center justify-between mt-1">
        <h2 class="text-2xl font-bold text-gray-900"><?= h($course['title']) ?></h2>
        <div class="flex items-center space-x-2">
            <?php if ($course['status'] === 'published'): ?>
                <a href="/course/<?= h($course['slug']) ?>" target="_blank" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition">
                    View Course &rarr;
                </a>
            <?php endif; ?>
            ...
            <a href="/admin/kurser/quiz/opret?course_id=<?= $course['id'] ?>" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition">
                + Add Quiz
            </a>
            <a href="/admin/certificates?course_id=<?= $course['id'] ?>" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition">
                Certificates
            </a>
        </div>
    </div>
    <p class="text-sm text-gray-500 mt-1"><?= (int)$course['total_lessons'] ?> lessons &middot; <?= $course['total_duration_seconds'] ? gmdate('H:i:s', $course['total_duration_seconds']) : '0:00' ?> total</p>
</div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <textarea id="short_description" name="short_description" rows="2"
                          class="w-full px-4 py-3 bg-white border border-gray-300 text-gray-900 rounded-lg focus:ring-2 focus:ring-indigo-500 text-sm"><?= h($course['short_description'] ?? '') ?></textarea>
            </div>
            <div class="md:col-span-2">
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6 mb-6">
        ...
        <div class="space-y-4">
            ...
            <div x-show="pricingType === 'one_time'" x-cloak class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Price (DKK)</label>
                    <input type="number" name="price_dkk" step="0.01" min="0" value="<?= h($course['price_dkk'] ?? '') ?>"
                           class="w-full px-4 py-2.5 bg-white border border-gray-300 text-gray-900 rounded-lg focus:ring-2 focus:ring-indigo-500 text-sm">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Compare Price (DKK)</label>
            ...
            <div x-show="pricingType === 'subscription'" x-cloak class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Monthly (DKK)</label>
                    <input type="number" name="subscription_price_monthly_dkk" step="0.01" min="0" value="<?= h($course['subscription_price_monthly_dkk'] ?? '') ?>"
                           class="w-full px-4 py-2.5 bg-white border border-gray-300 text-gray-900 rounded-lg focus:ring-2 focus:ring-indigo-500 text-sm">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Yearly (DKK)</label>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="file" name="instructor_image" accept="image/*"
                       class="w-full text-sm text-gray-500 file:mr-4 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-sm file:bg-indigo-600 file:text-white hover:file:bg-indigo-700">
            </div>
            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-700 mb-1.5">Bio</label>
                <textarea name="instructor_bio" rows="2" class="w-full px-4 py-3 bg-white border border-gray-300 text-gray-900 rounded-lg focus:ring-2 focus:ring-indigo-500 text-sm"><?= h($course['instructor_bio'] ?? '') ?></textarea>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6 mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <select name="status" class="w-full px-4 py-2.5 bg-white border border-gray-300 text-gray-900 rounded-lg focus:ring-2 focus:ring-indigo-500 text-sm">
            ...
            <div class="flex items-end space-x-6">
                <label class="flex items-center">
                    <input type="checkbox" name="is_featured" value="1" <?= $course['is_featured'] ? 'checked' : '' ?> class="w-4 h-4 text-indigo-600 bg-white border-gray-300 rounded focus:ring-indigo-500">
                    <span class="ml-2 text-sm text-gray-600">Featured</span>
                </label>
                <label class="flex items-center">
                    <input type="checkbox" name="drip_enabled" value="1" <?= $course['drip_enabled'] ? 'checked' : '' ?> class="w-4 h-4 text-indigo-600 bg-white border-gray-300 rounded focus:ring-indigo-500">
                    <span class="ml-2 text-sm text-gray-600">Drip content</span>
                </label>
            </div>
        </div>
    </div>
...
<div class="max-w-4xl">
    ...
    <div class="space-y-4" id="modules-list">
        ...
        <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden" x-data="{ open: true, editModule: false }">
            <!-- Module header -->
            <div class="flex items-center justify-between px-5 py-3 bg-gray-750 border-b border-gray-200 cursor-pointer" @click="open = !open">
                <div class="flex items-center space-x-3">
                    <svg class="w-4 h-4 text-gray-500 transition-transform" :class="open ? 'rotate-90' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                    <h4 class="text-sm font-semibold text-gray-900"><?= h($module['title']) ?></h4>
                    <span class="text-xs text-gray-500"><?= count($module['lessons']) ?> lessons</span>
                </div>
                <div class="flex items-center space-x-2" @click.stop>
                    <button @click="editModule = !editModule" class="text-xs text-gray-500 hover:text-gray-900 px-2 py-1 rounded hover:bg-gray-100 transition">Edit</button>
                    <form method="POST" action="/admin/kurser/modul/slet" onsubmit="return confirm('Delete this module and all its lessons?')">
                        ...
                        <button type="submit" class="text-xs text-red-600 hover:text-red-300 px-2 py-1 rounded hover:bg-gray-100 transition">Delete</button>
                    </form>
                </div>
            </div>

            <!-- Edit module inline form -->
            <div x-show="editModule" x-cloak class="px-5 py-3 bg-gray-50 border-b border-gray-200">
            ...
            <!-- Lessons list -->
            <div x-show="open" x-cloak class="divide-y divide-gray-200/50">
                <?php if (empty($module['lessons'])): ?>
                    <div class="px-5 py-4 text-sm text-gray-500 text-center">No lessons in this module yet.</div>
                <?php else: ?>
                    <?php foreach ($module['lessons'] as $lesson): ?>
                    <div class="flex items-center justify-between px-5 py-3 hover:bg-gray-50/50 transition group">
                        <div class="flex items-center space-x-3">
                            <?php
                            $typeIcons = [
                                'video' => '<svg class="w-4 h-4 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>',
                                'text' => '<svg class="w-4 h-4 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>',
                                'video_text' => '<svg class="w-4 h-4 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 4v16M17 4v16M3 8h4m10 0h4M3 12h18M3 16h4m10 0h4M4 20h16a1 1 0 001-1V5a1 1 0 00-1-1H4a1 1 0 00-1 1v14a1 1 0 001 1z"/></svg>',
                            ...
                            <?php if ($lesson['is_preview']): ?>
                                <span class="px-1.5 py-0.5 text-xs rounded bg-green-100 text-green-700">Preview</span>
                            <?php endif; ?>
                            <?php if ($lesson['video_status']): ?>
                                ...
                                $vsColor = $statusColors[$lesson['video_status']] ?? 'bg-gray-100 text-gray-700';
                                ?>
                        ...
                        <div class="flex items-center space-x-2 opacity-0 group-hover:opacity-100 transition">
                            ...
                            <?php endif; ?>
                            <a href="/admin/kurser/lektion?id=<?= $lesson['id'] ?>" class="text-xs text-gray-500 hover:text-gray-900 px-2 py-1 rounded hover:bg-gray-100 transition">Edit</a>
                        </div>
                    </div>
                    <?php endforeach; ?>
                <?php endif; ?>
                ...
                <div class="px-5 py-3">
                    <a href="/admin/kurser/lektion/opret?course_id=<?= $course['id'] ?>&module_id=<?= $module['id'] ?>"
                       class="inline-flex items-center text-xs text-indigo-600 hover:text-indigo-500 font-medium transition">
    ...
    <!-- Add Module Form -->
    <div class="mt-6 bg-white border border-gray-200 rounded-xl shadow-sm p-5" x-data="{ showForm: false }">
        ...
        <form x-show="showForm" x-cloak method="POST" action="/admin/kurser/modul/gem" class="space-y-3">
            ...
            <input type="hidden" name="course_id" value="<?= $course['id'] ?>">
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1">Module Title <span class="text-red-600">*</span></label>
                <input type="text" name="title" required class="w-full px-3 py-2 bg-white border border-gray-300 text-gray-900 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500"
                       placeholder="e.g. Module 1: Getting Started">
            </div>
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1">Description</label>
                <input type="text" name="description" class="w-full px-3 py-2 bg-white border border-gray-300 text-gray-900 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500"
            ...
            <div class="flex items-center space-x-2">
                ...
                <button type="button" @click="showForm = false" class="px-4 py-2 text-sm text-gray-500 hover:text-gray-900 transition">Cancel</button>
            </div>
        </form>
    </div>
</div>
...
<div class="max-w-4xl mt-8">
    <div class="flex items-center justify-between mb-4">
        <h3 class="text-xl font-bold text-gray-900">Quizzes (<?= count($courseQuizzes) ?>)</h3>
        <a href="/admin/kurser/quiz/opret?course_id=<?= $course['id'] ?>" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-indigo-600 hover:text-indigo-500 bg-white border border-gray-200 shadow-sm rounded-lg transition">
    ...
    <?php if (empty($courseQuizzes)): ?>
        <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6 text-center">
            <p class="text-gray-500 text-sm">No quizzes yet. Create a quiz to test students' knowledge.</p>
        </div>
    <?php else: ?>
        <div class="space-y-3">
            ...
            <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-4 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1">
                        ...
                        &middot; <?= $cq['status'] === 'published' ? '<span class="text-green-600">Published</span>' : '<span class="text-yellow-600">Draft</span>' ?>
                        <?php
                ...
                <div class="flex items-center space-x-3">
                    <a href="/admin/kurser/quiz/rediger?id=<?= $cq['id'] ?>" class="text-indigo-600 hover:text-indigo-500 text-xs font-medium">Edit</a>
