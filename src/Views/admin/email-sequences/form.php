...
<div class="mb-6">
    <a href="/admin/email-sequences" class="inline-flex items-center text-sm text-gray-500 hover:text-gray-900 transition">
...
<form method="POST" action="<?= $isEdit ? '/admin/email-sequences/update' : '/admin/email-sequences/store' ?>" class="space-y-8">
    ...
    <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6">
        <h3 class="text-lg font-semibold text-gray-900 mb-6">Sequence Settings</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="md:col-span-2">
                <label for="name" class="block text-sm font-medium text-gray-700 mb-2">Sequence Name *</label>
            ...
            <div>
                <label for="trigger_id" class="block text-sm font-medium text-gray-700 mb-2">Trigger ID <span class="text-gray-500">(optional)</span></label>
        ...
        <div class="flex items-center justify-end space-x-4 mt-6 pt-6 border-t border-gray-200">
            <a href="/admin/email-sequences" class="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition">
                Cancel
            </a>
...
<?php if ($isEdit): ?>
<!-- Steps Section -->
<div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6 mt-8" x-data="{ showAddStep: false, editingStep: null }">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h3 class="text-lg font-semibold text-gray-900">Sequence Steps</h3>
            <p class="text-sm text-gray-500 mt-1">Define the emails to send and when to send them.</p>
        </div>
        <button @click="showAddStep = !showAddStep; editingStep = null" type="button"
            class="inline-flex items-center px-4 py-2 text-sm font-medium text-indigo-600 bg-indigo-50 border border-indigo-200 hover:bg-indigo-100 rounded-lg transition">
    ...
    <?php if (empty($steps)): ?>
        <div class="text-center py-8" x-show="!showAddStep">
            ...
            <p class="text-gray-500 text-sm">No steps yet. Add your first email step to build the sequence.</p>
        </div>
    <?php else: ?>
        <div class="space-y-4">
            <?php foreach ($steps as $index => $step): ?>
            <div class="bg-gray-50 border border-gray-200 rounded-lg p-5" x-data="{ confirmDelete: false, editing: false }">
                <div class="flex items-center justify-between">
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            <p class="text-sm font-medium text-gray-900"><?= h($step['subject']) ?></p>
                ...
                <!-- Inline Edit Form -->
                <div x-show="editing" x-cloak class="mt-4 pt-4 border-t border-gray-200">
...
<div class="bg-white border border-gray-200 rounded-xl shadow-sm p-6 mt-8">
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Contacts currently in or who have completed this sequence.</p>
        </div>
        <span class="text-sm text-gray-500"><?= count($enrollments) ?> total</span>
    </div>

    <?php if (empty($enrollments)): ?>
        <div class="text-center py-8">
            <svg class="w-10 h-10 text-gray-600 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
            <p class="text-gray-500 text-sm">No enrollments yet. Contacts will appear here once they are enrolled in this sequence.</p>
        </div>
    <?php else: ?>
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-gray-200">
                        ...
                        <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Current Step</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-sm text-gray-600"><?= h($enrollment['name'] ?? '-') ?></td>
                        <td class="px-4 py-3">
                            <?php
                            $enrollStatusColors = [
                        ...
                        <td class="px-4 py-3 text-sm text-gray-500"><?= formatDate($enrollment['enrolled_at']) ?></td>
                        <td class="px-4 py-3 text-sm text-gray-500"><?= $enrollment['completed_at'] ? formatDate($enrollment['completed_at']) : '-' ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
